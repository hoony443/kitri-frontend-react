...
function Detail() {
    ...
    return(
        <div className={styles.DetailPage}>
            <div onClick={() => navigate(-1)} className={styles.back}>뒤로가기</div>
            {myMovie && (
                <>    
                    <div className={styles.thum}>
                        <img src={myMovie.large_cover_image} className={styles.img} alt="영화 포스터"></img>
                    </div>   
                    <div className={styles.inner}>
                        <div className={styles.detail}>영화 제목 
                            <span className={styles.detailspan}>{myMovie.title}</span>
                        </div>
                        <div className={styles.detail}>영화 ID 
                            <span className={styles.detailspan}>{myMovie.id}</span>
                        </div>
                        <div className={styles.detail}>영화 소개 
                            <span className={styles.detailspan}>{myMovie.description_full}</span>
                        </div>
                        <div className={styles.detail}>개봉 날짜 
                            <span className={styles.detailspan}>{myMovie.date_uploaded}</span>
                        </div>
                        <div className={styles.detail}>요약
